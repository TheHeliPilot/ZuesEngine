...
static void emit_header_once(LyncContext* ctx, const LyncApi* api) {
    ...
    api->emit_top(ctx,
        ...
        "    } \\\n"
        "    typedef void (*__zues_qe_cb_eng_##Name)(EntityRef e, Name* c, float dt); \\\n"
        "    static __zues_qe_cb_eng_##Name __zues_qe_user_eng_##Name = NULL; \\\n"
        "    static void __zues_qe_thunk_eng_##Name(ZuesEntity e, void** column_ptrs, \\\n"
        "                                           uint32_t column_count, void* user) { \\\n"
        "        (void)user; (void)column_count; \\\n"
        "        if (__zues_qe_user_eng_##Name && column_ptrs && column_ptrs[0]) \\\n"
        "            __zues_qe_user_eng_##Name(__zues_from_z(e), (Name*)column_ptrs[0], __zues_dt); \\\n"
        "    } \\\n"
        "    void Each__##Name(void* cb) { \\\n"
        "        ZuesComponentId cid = __zues_resolve_engine_##Name(); \\\n"
        "        if (!cid || !__zues_host || !__zues_host->query_each || !cb) return; \\\n"
        "        /* Save/restore so a nested Each<T> doesn't clobber the outer. */ \\\n"
        "        __zues_qe_cb_eng_##Name prev = __zues_qe_user_eng_##Name; \\\n"
        "        __zues_qe_user_eng_##Name = (__zues_qe_cb_eng_##Name)cb; \\\n"
        "        ZuesComponentId ids[1] = { cid }; \\\n"
        "        __zues_host->query_each(__zues_engine, ids, 1, NULL, 0, \\\n"
        "                                __zues_qe_thunk_eng_##Name, NULL); \\\n"
        "        __zues_qe_user_eng_##Name = prev; \\\n"
        "    }\n"
