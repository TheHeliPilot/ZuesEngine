...
// (Zues patch) Public-API delegates so external callers (autocomplete
// detection, snippet helpers) can convert between visual columns and
// character indices without poking at private methods.
int TextEditor::VisualColumnToCharacterIndex(int aLine, int aColumn) const
{
	return GetCharacterIndex(Coordinates(aLine, aColumn));
}
int TextEditor::CharacterIndexToVisualColumn(int aLine, int aIndex) const
{
	return GetCharacterColumn(aLine, aIndex);
}
...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z))
			...
			// (Zues patch) Honour one-shot Enter suppression so the host
			// can intercept Enter for autocomplete commit without the
			// editor inserting a newline.
			if (!mSuppressEnterOnce) EnterCharacter('\n', false);
		}
		else if (!IsReadOnly() && !ctrl && !alt && ImGui::IsKeyPressed(ImGuiKey_Tab)) {
			// (Zues patch) Honour one-shot Tab suppression (autocomplete
			// commit, snippet expansion). Drops the literal '\t' that
			// would otherwise be inserted before the host can run.
			if (!mSuppressTabOnce) EnterCharacter('\t', shift);
		}

		// (Zues patch) Auto-clear the one-shot suppression flags. Caller
		// must re-set them every frame they want the keys swallowed.
		mSuppressEnterOnce = false;
		mSuppressTabOnce   = false;
...
void TextEditor::Render()
{
	...
	// (Zues patch) Stash the screen origin of the content area so external
	// overlays (autocomplete ghost text, bracket-pair highlight, ...) can
	// compute the screen position of any character cell after Render().
	// Don't subtract scroll - cursorScreenPos already sits at "where line 0
	// would draw" (which is above the visible area when scrolled). The
	// renderer uses cursorScreenPos.y + line * lineHeight directly for each
	// line; we mirror that math in GetCharScreenPos.
	mLastContentScreenPos = cursorScreenPos;
	...
	// (Zues patch) Pin to a minimum 4-digit gutter so the editor body doesn't
	// jump horizontally when the line count crosses 9->10, 99->100 etc. The
	// gutter still grows for files with >9999 lines.
	char buf[16];
	const int width_for = std::max(globalLineMax, 9999);
	snprintf(buf, 16, " %d ", width_for);
		...
		while (lineNo <= lineMax)
		{
			...
			if (mState.mCursorPosition.mLine == lineNo)
			{
				...
				if (focused)
				{
					...
					// (Zues patch) Reset the blink phase whenever the
					// caret has just moved — keeps the cursor solidly
					// visible right after navigation so the user always
					// sees where they landed. Without this, fast arrow
					// keying lands inside the OFF half of the blink and
					// the caret seems to disappear.
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	// (Zues patch) Drop the always-on horizontal scrollbar when word-wrap
	// is requested. Full visual line-wrap is not yet implemented; this is
	// the minimum no-H-scroll variant.
	const ImGuiWindowFlags child_flags = mWordWrap
		? (ImGuiWindowFlags_NoMove)
		: (ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysHorizontalScrollbar | ImGuiWindowFlags_NoMove);
	if (!mIgnoreImGuiChild)
		ImGui::BeginChild(aTitle, aSize, aBorder, child_flags);
	...
	// (Zues patch) Bracket-pair highlight overlay. Drawn after the main
	// text render so it sits ON TOP of the glyphs. Cheap: looks at the
	// caret's neighbours, walks at most one direction to find the match.
	if (mMatchPairsHighlight)
	{
		auto pos = GetActualCursorCoordinates();
		if (pos.mLine >= 0 && pos.mLine < (int)mLines.size())
		{
			const auto& line = mLines[pos.mLine];
			auto bracket_at = [&](int col, char& out_c) -> bool {
				if (col < 0 || col >= (int)line.size()) return false;
				const char c = (char)line[col].mChar;
				if (c=='{'||c=='}'||c=='('||c==')'||c=='['||c==']') { out_c = c; return true; }
				return false;
			};
			char c = 0;
			int  bcol = -1;
			// Prefer the bracket on the LEFT of the caret (matches IDE feel).
			if (pos.mColumn > 0 && bracket_at(pos.mColumn - 1, c)) bcol = pos.mColumn - 1;
			else if (bracket_at(pos.mColumn, c))                    bcol = pos.mColumn;
			if (bcol >= 0)
			{
				const bool opening = (c=='{'||c=='('||c=='[');
				const char open_c  = opening ? c : (c=='}'?'{':(c==')'?'(':'['));
				const char close_c = opening ? (c=='{'?'}':(c=='('?')':']')) : c;
				const int  dir     = opening ? +1 : -1;
				int line_i = pos.mLine, col_i = bcol + dir;
				int depth = 1;
				bool found = false;
				int  match_line = -1, match_col = -1;
				// Walk forward (or back) across glyphs respecting depth.
				// `found` ends both loops without a goto.
				while (!found && line_i >= 0 && line_i < (int)mLines.size())
				{
					const auto& cur = mLines[line_i];
					while (col_i >= 0 && col_i < (int)cur.size())
					{
						const char cc = (char)cur[col_i].mChar;
						if (cc == open_c)  depth += (dir > 0) ? 1 : -1;
						else if (cc == close_c) depth += (dir > 0) ? -1 : 1;
						if (depth == 0) { match_line = line_i; match_col = col_i; found = true; break; }
						col_i += dir;
					}
					if (found) break;
					line_i += dir;
					if (line_i < 0 || line_i >= (int)mLines.size()) break;
					col_i = (dir > 0) ? 0 : (int)mLines[line_i].size() - 1;
				}
				auto* dl = ImGui::GetWindowDrawList();
				const ImU32 col_hi = IM_COL32(220, 220, 80, 90);   // soft yellow box
				// Exact char width via TextDistanceToLineStart delta.
				// mCharAdvance.x is just the width of '#' - inaccurate for
				// variable-width fonts on glyphs like '{'. The delta below
				// matches whatever the renderer actually drew.
				auto draw_box = [&](int line_no, int col_no) {
					const float x0 = TextDistanceToLineStart(Coordinates(line_no, col_no));
					const float x1 = TextDistanceToLineStart(Coordinates(line_no, col_no + 1));
					ImVec2 p = GetCharScreenPos(Coordinates(line_no, col_no));
					ImVec2 q = ImVec2(p.x + (x1 - x0), p.y + mCharAdvance.y);
					dl->AddRectFilled(p, q, col_hi, 2.0f);
				};
				draw_box(pos.mLine, bcol);
				if (found) draw_box(match_line, match_col);
			}
		}
	}
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					if (!line[first - bufferBegin].mPreprocessor)
					{
						if (mLanguageDefinition.mKeywords.count(id) != 0)
							...
							token_color = PaletteIndex::TypeName;
...
void TextEditor::ColorizeInternal()
{
	...
	// (Zues patch) Rainbow brackets pass. Walk all glyphs in source order,
	// track shared bracket-nesting depth across {, (, [ and reassign
	// mColorIndex on each bracket glyph to one of the RainbowBracket0..4
	// palette slots. Cheap (O(N) glyphs), idempotent. No-op when disabled.
	if (mRainbowBrackets)
	{
		int depth = 0;
		for (auto& line : mLines)
		{
			for (auto& g : line)
			{
				...
				if (c == '{' || c == '(' || c == '[')
				{
					const int slot = depth % 5;
					g.mColorIndex = (PaletteIndex)((int)PaletteIndex::RainbowBracket0 + slot);
					++depth;
				}
				else if (c == '}' || c == ')' || c == ']')
				{
